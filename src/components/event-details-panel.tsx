...
export function EventDetailsPanel({ event, onClose }: EventDetailsPanelProps) {
  ...
  const matching = event ? filtersMatchingEvent(filters, event) : []

  useEffect(() => {
    if (!open) return
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape" && !creatingFilter) onClose()
    }
    window.addEventListener("keydown", onKey)
    return () => window.removeEventListener("keydown", onKey)
  }, [open, onClose, creatingFilter])
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed top-0 right-0 z-50 flex h-svh w-full max-w-md flex-col border-l border-border bg-background shadow-xl transition-transform",
